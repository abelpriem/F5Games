...
public class BackpackService {
    ...
    public Backpack insertOne(Long objectId) {
        if(objectId==null){
            throw new InvalidDataException("GameObject id cannot be null");
        }
        if(backpackRepository.count()>=3){
            throw new InvalidDataException("Backpack is full");
        }
        GameObject gameObject = gameObjectRepository.findById(objectId).orElseThrow(() -> new InvalidDataException("GameObject not found with id "+objectId));
    ...
    public Backpack updateOne(Long id, Long objectId) {
        if(id==null){
            throw new InvalidDataException("Backpack identifier cannot be null");
        }
        if(objectId==null){
            throw new InvalidDataException("GameObject id cannot be null");
        }
        backpackRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Backpack identifier "+id+" not found"));
        GameObject gameObject = gameObjectRepository.findById(objectId).orElseThrow(() -> new InvalidDataException("GameObject not found with id "+objectId));
    ...
    public void deleteOne(Long id) {
        ...
        Backpack backpack = backpackRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Backpack identifier "+id+" not found"));
